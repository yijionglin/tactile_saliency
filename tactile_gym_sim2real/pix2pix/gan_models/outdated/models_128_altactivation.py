...
class GeneratorUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # no skips
        # u1 = self.up1(d7, torch.zeros(d6.shape).cuda())
        # u2 = self.up2(u1, torch.zeros(d5.shape).cuda())
        # u3 = self.up3(u2, torch.zeros(d4.shape).cuda())
        # u4 = self.up4(u3, torch.zeros(d3.shape).cuda())
        # u5 = self.up5(u4, torch.zeros(d2.shape).cuda())
        # u6 = self.up6(u5, torch.zeros(d1.shape).cuda())
