...
discriminator = Discriminator(in_channels=opt.channels, act_dim=rl_model.action_space.shape[0])
...
# check if border images need to be added for RL and load them as tensors
if rl_params['add_border']:
    rl_image_size_str = str(rl_params['image_size'][0]) + 'x' + str(rl_params['image_size'][1])
    ...
    border_gray = Tensor(np.load(border_gray_savefile))
    border_mask = Tensor(np.load(border_mask_savefile))
...
def check_against_numpy_predict(images, actions):

    images_array = images.detach().cpu().numpy()

    numpy_actions, _ = rl_model.predict(images_array, deterministic=deterministic)
    torch_actions = actions.detach().cpu().numpy()

    print('')
    print('')
    print((numpy_actions == torch_actions).all())

def get_rl_info(obs):
    """
    Forward pass in all the networks (actor and critic)
    """
    latent_pi, latent_vf, latent_sde = rl_model.policy._get_latent(obs) # calls extract_features which applies preprocessing
    values = rl_model.policy.value_net(latent_vf)
    distribution = rl_model.policy._get_action_dist_from_latent(latent_pi, latent_sde=latent_sde)
    actions = distribution.get_actions(deterministic=deterministic)
    log_prob = distribution.log_prob(actions)
    return actions, values, log_prob, distribution

def differentiable_predict(images):
    """
    Prep image as expected by RL networks, pass through and predict actions whilst
    allowing backprop.

    image input format (batch, C, H, W)
    """

    # resize to expected by RL
    if list(augmentation_params['dims']) != list(rl_params['image_size']):
        images = F.interpolate(images, size=rl_params['image_size'])

    # repeat for n_frames here
    if rl_params['n_stack'] > 1:
        images = torch.repeat_interleave(images, rl_params['n_stack'], dim=1)

    # convert to image format (as normalisation is auto applied)
    images = torch.clamp(images, 0, 1) * 255

    # attach a border
    if rl_params['add_border']:
        images[:,:,border_mask==1] = border_gray[border_mask==1]

    # visualise the images before sending to RL net
    # plot_obs_stack(images.detach().cpu().numpy())

    # get the rl info
    actions, values, log_prob, distribution = get_rl_info(images)

    # this assumes the same action bounds for all actions
    actions = torch.clamp(actions, rl_model.action_space.low[0], rl_model.action_space.high[0])

    # This may fail when using augmentation in custom rl network
    # check_against_numpy_predict(images, actions)
...
loss_df = pd.DataFrame(columns=['Epoch', 'D_Loss', 'Real_Loss', 'Fake_Loss', 'Aux_Loss', 'G_Loss', 'GAN_Loss', 'Pixel_Loss', 'RL_Loss'])
...
for epoch in range(opt.epoch, opt.n_epochs+1):
    for i, batch in enumerate(training_loader):
        ...
        pred_gen, _ = discriminator(gen_sim_images, tip_images)
        ...
        # RL loss
        sim_actions, sim_values, sim_log_prob = differentiable_predict(sim_images)
        gen_sim_actions, gen_sim_values, gen_sim_log_prob = differentiable_predict(gen_sim_images)

        # loss_rl = criterion_rl(gen_sim_actions, sim_actions)
        loss_rl = criterion_rl(gen_sim_values, sim_values) + criterion_rl(gen_sim_actions, sim_actions)

        # Total loss
        loss_G = (weights['W_gan']*loss_GAN) + (weights['W_pixel']*loss_pixel) + (weights['W_rl']*loss_rl)
        ...
        # Real loss
        pred_real, pred_real_actions = discriminator(sim_images, tip_images)
        loss_real_actions = criterion_rl(pred_real_actions, sim_actions.detach())
        loss_real = criterion_GAN(pred_real, valid)

        # Fake loss
        pred_fake, pred_fake_actions = discriminator(gen_sim_images.detach(), tip_images)
        loss_fake_actions = criterion_rl(pred_fake_actions, gen_sim_actions.detach())
        loss_fake = criterion_GAN(pred_fake, fake)

        # Total loss
        loss_aux = 0.5 * (loss_real_actions + loss_fake_actions)
        loss_disc = 0.5 * (loss_real + loss_fake)
        loss_D = loss_disc + (weights['W_aux'] * loss_aux)
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch {}/{}] [Batch {}/{}] [D_loss: {:.5f}, real_loss: {:.5f}, fake_loss: {:.5f}, aux_loss: {:.5f}] [G_loss: {:.5f}, GAN_loss: {:.5f}, pix_loss: {:.5f}, rl_loss: {:.5f}] ETA: {}".format(
                epoch,
                opt.n_epochs,
                i,
                len(training_loader),
        ...
        running_losses[3] += loss_aux.item()
        running_losses[4] += loss_G.item()
        running_losses[5] += loss_GAN.item()
        running_losses[6] += loss_pixel.item()
        running_losses[7] += loss_rl.item()
    ...
    if (epoch % opt.sample_interval == 0) or ((epoch) % opt.n_epochs == 0):
        ...
        # check if this has the lowest running RL loss
